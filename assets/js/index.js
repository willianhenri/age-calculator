document.addEventListener('DOMContentLoaded', () => {
  ...
  function calculateAge() {
    ...
    if (!isOnlyNumbers(dayInput.value) || !isOnlyNumbers(monthInput.value) || !isOnlyNumbers(yearInput.value)) {
      if (!isOnlyNumbers(dayInput.value)) {
        showError(dayInput, dayLabel, dayError, "Por favor, insira um dia válido.");
      }
      if (!isOnlyNumbers(monthInput.value)) {
        showError(monthInput, monthLabel, monthError, "Por favor, insira um mês válido.");
      }
      if (!isOnlyNumbers(yearInput.value)) {
    ...
    if (birthDate > today) {
      showError(yearInput, yearLabel, yearError, "A data de nascimento não pode ser no futuro.");
      return;
    }
